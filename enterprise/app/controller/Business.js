Ext.define('SP.controller.Business', {
        ...
        this.control({
        	...
        	"businesslist panel[itemId=complexQueryPanel] combobox" : {
        		...
        		render : function(comp){
         		   ...
         		   store.load(function(){
        			   comp.on('change' , me.queryBusinessByComplex , me);
        		   });
        		}
        	},
            "businesslist > grid" : {
    ...
    queryBusinessByKeyword : function(comp){
    	...
    	store.getProxy().api.read = SP.GlobalConfig.getBaseUrl() + "/platform/admin?actid=1076";
    ...
    queryBusinessByComplex : function(comp){
    	...
    	var grid = this.getBusinessListGrid();	
    	var store = grid.getStore();
    	store.getProxy().api.read = SP.GlobalConfig.getBaseUrl() + "/platform/admin?actid=1077";
    	store.getProxy().extraParams = {
    		"treetype" : treeType,
